const jatekAdatok = {
    'fifa': {
        cim: "EA Sports FC (FIFA)",
        leiras: "Az EA Sports FC a világ legnépszerűbb futballszimulátora, amelyben a realizmus és a licencelt csapatok sokasága dominál. A játékosok saját klubot építhetnek az Ultimate Team módban, vagy átélhetik egy profi focista karrierjét a menedzser módban is. A modern technológiáknak köszönheden a mozgások és a labda fizikai viselkedése évről évre élethűbbé válik. Legyen szó baráti meccsekről vagy komoly online rangsorolt küzdelmekről, a játék minden stílushoz kínál megfelelő kihívást. Ez a sorozat évtizedek óta a sportjátékok megkerülhetetlen királya, amely összehozza a rajongókat a világ minden tájáról.",
        kep: "img/ea-fc-26.jpg" 
    },
    'gta': {
        cim: "Grand Theft Auto V",
        leiras: "A Rockstar Games alkotása egy monumentális nyílt világú kaland, amely Los Santos napfényes és bűnnel teli utcáin játszódik. A történet különlegessége, hogy three különböző hátterű bűnöző sorsát irányíthatjuk egyszerre, akiknek útjai egy nagyszabású rablássorozatban fonódnak össze. A játékosok előtt álló szabadság szinte határtalan, legyen szó repülők vezetéséről, tengeri búvárkodásról vagy egyszerű utcai versenyekről. A részletgazdag környezet és a szatirikus humor egyedülálló módon mutatja be a modern amerikai társadalom paródiáját. A több mint egy évtizede töretlen népszerűségnek örvendő Online mód pedig folyamatosan frissülő tartalmakkal tartja lázban a közösséget.",
        kep: "img/gta5.avif"
    },
    'dmc': {
        cim: "Devil May Cry 5",
        leiras: "A Devil May Cry 5 a stílusos akciójátékok koronázatlan királya, ahol a démonvadászat minden pillanata tiszta adrenalin. A játékosok három karakter, Nero, Dante és a titokzatos V egyedi harcstílusát sajátíthatják el a történet során. A harcrendszer középpontjában a stílusértékelés áll, ami arra ösztönzi, hogy minél változatosabb és látványosabb kombókkal semmisítsük meg az ellenfeleket. A fotorealisztikus grafika és a dübörgő heavy metal zene tökéletes összhangot teremt a brutális összecsapások közben. Ez a rész méltó lezárása és egyben megújítása a sorozatnak, amely kihívást és esztétikai élményt is nyújt a veteránoknak.",
        kep: "img/DMC5.webp"
    },
    'rdr': {
        cim: "Red Dead Redemption 2",
        leiras: "Ez a western eposz Arthur Morgan és a Van der Linde banda utolsó napjait meséli el a vadnyugat alkonyán. A Rockstar Games eddigi legrészletesebb világa elképesztő vizualitással és egy hihetetlenül élő ökoszisztémával várja a felfedezőket. A langsam tempójú, karakterközpontú történetvezetés mély érzelmi hatást gyakorol a játékosra, miközben erkölcsi döntéseket kell hoznia. A vadászat, a horgászat és a tábori élet apró részletei mind hozzájárulnak ahhoz, hogy valóban egy 19. századi törvényen kívüli bőrében érezzük magunkat. Nem csupán egy játékról, hanem egy interaktív művészeti alkotásról van szó, amely felejthetetlen élményt nyújt.",
        kep: "img/rdr2.png"
    },
    'yakuza': {
        cim: "Yakuza (Like a Dragon)",
        leiras: "A Yakuza-sorozat egyedülálló módon ötvözi a komoly japán maffia-drámát a legmeghökkentőbb, sokszor abszurd humorral. Kamurocho neonfényes negyedében az utcai verekedések mellett számtalan szórakoztató minijáték, például karaoke és játéktermek várják a látogatókat. A karakterek kidolgozottsága és a hűségre épülő történetek mély bepillantást engednek a japán alvilág belső szabályrendszerébe. A legutóbbi részek már körökre osztott harcrendszert is bevezettek, ami teljesen új színt hozott a klasszikus akció dús formulába. Ez a franchise azoknak készült, akik szeretik az érzelmes történeteket, de nem riadnak vissza a japán popkultúra őrültségeitől sem.",
        kep: "img/Yakuza.jpg"
    },
    'cyberpunk': {
        cim: "Cyberpunk 2077",
        leiras: "Night City futurisztikus metropolisza a technológiai fejlődés és a társadalmi hanyatlás lenyűgöző, neonfényben úszó elegye. A játékos V, egy zsoldos szerepébe bújva küzd az életéért, miközben egy digitális szellem, Johnny Silverhand kísérti az elméjét. A karakterfejlesztés során kiberverekkel és különböző képességekkel szabhatjuk testre harcmodorunkat a saját ízlésünknek megfelelően. A döntéseink alapvetően befolyásolják a város sorsát és a különböző frakciókkal való viszonyunkat a szerteágazó történetben. A rengeteg javítás és az új kiegészítő után a játék végre teljes pompájában ragyog, és az egyik legjobb RPG élményt nyújtja.",
        kep: "img/cyberpunk2077.jpg"
    },
    'godofwar': {
        cim: "God of War Ragnarök",
        leiras: "Kratos és Atreus északi kalandja a végkifejletéhez ér, ahol a kilenc birodalom sorsa dől el az istenek alkonyakor. Az apa és fia közötti dinamikus kapcsolat fejlődése adja a történet igazi lelkét, miközben a próféciákkal dacolnak. A brutális és taktikus harcrendszerben a Leviatán-balta és a Káosz-pengék mellett Atreus nyilaival is hatékonyan kell operálnunk. A látványvilág minden birodalomban lélegzetelállító, legyen szó jégmezőkről vagy buja erdőkről, amelyeket mitikus lények népesítenek be. Ez a játék mesterien vegyíti az epikus akciót a mélyen emberi érzelmekkel, lezárva egy korszakot a videojáték-történelemben.",
        kep: "img/GodOfWar.jpg"
    },
    'eldenring': {
        cim: "Elden Ring",
        leiras: "A FromSoftware és George R. R. Martin együttműködéséből született mű a modern nyílt világú soulslike játékok abszolút csúcspontja. A Lands Between hatalmas területei titkokat, félelmetes főellenségeket és lélegzetelállító tájakat rejtenek minden felfedező számára. A játékosok teljes szabadságot gapnak abban, hogyan építik fel karakterüket, legyen szó varázslatról vagy nyers fizikai erőről. A kihívás hatalmas, de minden győzelem olyan sikerélményt ad, amellyel kevés más játék tud versenyezni a piacon. Ez a sötét fantasy világ minden egyes felfedezett barlanggal és legyőzött szörnyeteggel mélyebbre rántja a játékost a saját mitológiájába.",
        kep: "img/eldenRing.png"
    }
};

function tartalomCsere(jatekKulcs) {
    const leirasDoboz = document.getElementById('jatek_leiras_doboz');
    const kepMegjelenito = document.getElementById('fokep_megjelenito');

    if (jatekAdatok[jatekKulcs]) {
        const adat = jatekAdatok[jatekKulcs];
        
        leirasDoboz.innerHTML = `
            <h2 class="text-info mb-3">${adat.cim}</h2>
            <p style="font-size: 1.1rem; line-height: 1.7; color: #e0e0e0;">${adat.leiras}</p>
        `;
        
        if (adat.kep) {
            kepMegjelenito.src = adat.kep;
            kepMegjelenito.style.display = "block";
            
            kepMegjelenito.style.opacity = "0";
            setTimeout(() => {
                kepMegjelenito.style.transition = "opacity 0.4s ease-in";
                kepMegjelenito.style.opacity = "1";
            }, 50);
        }
    }
}

function szavazas(jatekKulcs) {
    const szamlalo = document.getElementById(`${jatekKulcs}-szavazat`);
    if (szamlalo) {
        let aktualisErtek = parseInt(szamlalo.innerText);
        szamlalo.innerText = aktualisErtek + 1;

        const visszajelzes = document.getElementById('szavazas-feedback');
        visszajelzes.style.display = 'block';
        setTimeout(() => {
            visszajelzes.style.display = 'none';
        }, 2000);
    }
}

window.addEventListener('DOMContentLoaded', () => {
    const welcomeModalEl = document.getElementById('welcomeModal');
    if (welcomeModalEl) {
        const myModal = new bootstrap.Modal(welcomeModalEl);
        myModal.show();
    }
});
function hirlevelKezelo(event) {
    event.preventDefault();
    const inputmezo = document.getElementById('newsEmail');
    const visszajelzes = document.getElementById('newsletter-feedback');
    
    if (inputmezo && inputmezo.value.trim() !== "") {
        visszajelzes.style.display = 'block';
        inputmezo.value = "";
        setTimeout(() => {
            visszajelzes.style.display = 'none';
        }, 2500);
    }
}

function foglalasKezelo(event) {
    event.preventDefault();
    alert("Köszönjük! A foglalási igényedet rögzítettük, hamarosan küldjük a visszaigazolást e-mailben!");
    
    const bookingModalEl = document.getElementById('bookingModal');
    const modalInstance = bootstrap.Modal.getInstance(bookingModalEl);
    if (modalInstance) {
        modalInstance.hide();
    }
    event.target.reset(); 
}